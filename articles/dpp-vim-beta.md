...
その後、プラグインの遅延起動が当然のようになっていくのは皆さんが記憶にある通りです。
:::
...
他には高速なプラグインのインストールも特徴として挙げられると思います。`vim-plug` が開発されてから既に 10 年ほど経ちますが、今だにファンが多いプラグインマネージャーです。
...
`dein.vim` は機能が多いのでちゃんと設定するのは難しいのですが、適切な設定さえできればプラグインマネージャー最速クラスの速度を叩き出すことが可能です。
