...
その後 Vim への `cmdheight = 0` パッチの移植作業を進めていました。これは思っていたよりも簡単に動作したからです。
...
Vim/neovim のコマンドラインはメッセージ機能としても使用されており、これを無効化する場合は代用の機能を用意する必要があります。
Vim はメッセージをポップアップとして表示する機能 `:echowindow` を実装しましたが、neovim ではメッセージ UI をプラグイン側で実装できるようになる予定です。
...
これらの問題を根本的に対応するには、コマンドライン機能、メッセージ機能を外に出す他ないと思われます。neovim はそれを実装予定です。
